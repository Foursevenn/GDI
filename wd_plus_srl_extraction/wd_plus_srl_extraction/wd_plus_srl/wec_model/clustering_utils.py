...
from wd_plus_srl_extraction.wd_plus_srl.dataobjs.cluster import Clusters

logger = logging.getLogger(__name__)


def agglomerative_clustering(pred_matrix, mentions, average_link_thresh):
    start = time.time()
    total_merged_clusters = run_clustering(pred_matrix, mentions, average_link_thresh)
...
def run_clustering(pred_matrix, mentions, average_link_thresh):
    ...
    for i in range(len(mentions)):
        mentions[i].predicted_coref_chain = fit.labels_[i] + Clusters.get_cluster_coref_chain()
